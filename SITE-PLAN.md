...
**Status.** PR 1: the machinery and Recipes 1–13. `exercises/cookbook/rust/` is a dependency-free crate, one module per cookbook domain, each recipe between the same `recipe-N` markers the C++ files use and each module's tests asserting the recipe's claims (nine tests, including Recipe 7's C handle through `extern "C"` and Recipe 13's panic surfacing at `join`). On the page each of the thirteen recipes is two tabs, C++ and Rust, both includes; four Why paragraphs close with one **In Rust** sentence where the language sharpens the point (a missing file is an `Err`; `Drop` is the destructor; `get` never inserts; `join` carries the panic in its type). `build_all.sh` runs `cargo test --offline` with warnings as errors behind a `--require-cargo` probe that CI passes. PR 2: Recipes 14–35 — eleven more modules and eighteen more tests (the event source and the timer whose `Drop` joins, the UTF conversions with their lossy repair, the error type with `Display`, `Result` as the standard type, the flags newtype, the shared sink behind `Rc<RefCell>`, the 4 MB frame built on a 256 KB thread stack to prove it never touches the stack); four recipes carry a one-line Rust note instead of code, because the standard library has no JSON and no calendar and the crate takes no dependency (25, 26, 35, 29). PR 3: Recipes 36–49 — the atomic save, the export rotation, the polling watcher (`fs::metadata` answers all three questions, absence is a state), the pattern that needs no regex, and trim/compare/prefix/suffix; nine notes for the recipes whose C++ half is OpenSSL, libcurl, sqlite3, shared memory or a memory map, each naming the ecosystem crate and saying why the crate does not take it. Every recipe now has the tab: 36 with code and 32 tests, 13 with a note. PR 4: the other two items. Seven concept chapters (1, 4, 6, 7, 8, 10, 41) close with a short **In Rust** section placed before their last section — ownership as the type system's subject, no inheritance, the Rule of Zero as the language, generics as checked monomorphization, one error dialect with `?`, the fluency chapter as Rust's defaults, the seam as a plain generic — never woven into the C++ explanation. Chapter 30 gains *In Rust: the same seam, from the caller's side*: `exercises/abilab/rust_client/` is a dependency-free crate whose build script compiles `engine.cpp` and links it, whose `lib.rs` declares the three façade functions again and wraps the handle in `Drop`, and whose test is `engine_demo.cpp`'s assertions in Rust; `build_all.sh` runs it under the cargo probe, and the chapter includes the declarations and the wrapper. Chapters 38 and 39 point at it as one more consumer of the C ABI. Step 5 is DONE, and with it the plan's five steps; the open decisions below remain open.
