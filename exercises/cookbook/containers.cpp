...
// the resize-then-push_back trap. The other trap, reserve-then-index, is
// undefined behavior and stays a comment.
#include <cassert>
#include <cstddef>
#include <vector>
...
int main() {
    ...
    // moves the block - the same data() pointer before and after.
    ...
    // A reserve smaller than the current capacity is a no-op, which is why a
    // reserve inside a loop reserves nothing new.
    ...
    // shape), because both mainstream libraries annotate a vector's unused
    // capacity under ASan.
    return 0;
}
