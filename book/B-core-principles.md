...
- "A closed set of unrelated alternatives is a std::variant — stored by value, visited exhaustively, and the compiler refuses a visitor that forgets one; an open set someone else extends is a virtual base behind unique_ptr."
...
- "A named rvalue reference is an lvalue and a const object cannot be stolen from — so I write std::move inside every move operation, never around a return, and never expect it to move a const."
