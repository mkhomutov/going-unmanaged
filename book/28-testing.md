...
Read that carefully, because the obvious reading is wrong. A double-free report has **three** stacks — the free it stopped, the free that came first, and the allocation — and each one is a single call chain, so no stack here contains two deletes. The pair of `Buffer::~Buffer()` frames is not two destructions; it is *one*. The Itanium C++ ABI gives every destructor two entry points — a complete-object one and a base-object one — and on this macOS/clang build they are two separate functions even for a class like this with no base at all, the first simply calling the second unoptimized. (On Linux the toolchains usually emit the two as one body under two names, so a single frame appears; MSVC's ABI has one destructor entry point and the doubling cannot appear there at all.) The tell is in the third stack, where the same doubling happens to `Buffer::Buffer` — and a constructor plainly ran only once. Rebuild at `-O1` and the pairs do not halve, they vanish: both entry points are inlined into their callers, no `Buffer` frame survives at all, and the summary line moves off `Buffer.h:20` onto the test function. The bug has not moved an inch; only the frames describing it have. Which is the lesson under the lesson — a frame count is a fact about your toolchain and optimization level, never about how many times something ran.
...
- **A test suite that cannot fail.** Break the code on purpose once and confirm the suite goes red. An untested test is not evidence of anything — this is predict-then-run applied to your own harness.
