...
So: two deletes, both at `buffer_test.cpp:51`, the closing brace where `a` and `moved` go out of scope in that order — and the allocation stack names line 45, `Buffer a(3)`, the block they now both think they own. Exit code 134: the process aborted rather than finishing.
