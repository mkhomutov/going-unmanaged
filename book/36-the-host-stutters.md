...
- **Solving the deadline and forgetting the lifetime.** This chapter is about what a callback may *do*; [Chapter 29](29-concurrency.md#chapter-29--concurrency) is about whether the object it points at is still there. A tick handler that allocates nothing and still holds a raw pointer to something the host can destroy has traded a stutter for a crash. Both chapters describe the same callback from different sides, and a real one needs both answers.
...
- **Needing data from another thread, and having only a mutex to get it.** This ticket's meter borrows from buffers the host already owns, so nothing crosses a thread. The day the deadline thread needs samples a *worker* produced, Chapter 29's mutex is the wrong tool for the reason above, and [Chapter 43](43-below-the-mutex.md#chapter-43--below-the-mutex) is the right one: a bounded ring with one producer and one consumer, which never waits on the deadline side.
