...
Both smart pointers take a **custom deleter**, which is the RAII guard for a resource whose release is one function call: `std::unique_ptr<std::FILE, int (*)(std::FILE*)>` with `&std::fclose` — the shape Recipe 7 in [Appendix F](F-rosetta-cookbook.md#appendix-f--the-rosetta-cookbook) spells out — or `std::shared_ptr<ThingRef>(raw, Thing_Release)` for a refcounted handle. The deleter is part of the pointer's type for `unique_ptr` and captured at construction for `shared_ptr`, and it runs *instead of* `delete`, which is what makes a pointer from a C API safe to hold at all: `delete` on a block the SDK allocated is Chapter 17's two-heaps bug. Reach for the deleter when the release is one call; write the class below when there is more to manage than that.

The odd one out is `std::enable_shared_from_this<T>`: it lets an object hand out a `shared_ptr` — or, more usefully, a `weak_ptr` — to *itself* from inside a member function, provided it is already owned by a `shared_ptr`. It exists for exactly the situation [Chapter 29](29-concurrency.md#chapter-29--concurrency) builds by hand with a separate control block, a callback that must ask "is the thing I point at still alive?" without being handed a raw `this`. Calling it on an object no `shared_ptr` owns throws `bad_weak_ptr` since C++17; before that it was undefined behaviour, which is the version you will meet in older code.
