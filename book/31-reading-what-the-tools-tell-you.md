...
| A trimmed or matched substring reads as garbage a few lines later — or a config value parsed fine and the per-sample path with a pattern in it stutters | Recipes 44 and 45 in [Appendix F](F-rosetta-cookbook.md#appendix-f--the-rosetta-cookbook): a view or a `match_results` into a string that has died; `std::regex` on the deadline path |
