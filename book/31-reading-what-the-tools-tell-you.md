...
- **macOS: `sample`, then Instruments.** `sample <pid> 10` is on every Mac, needs no Xcode, and attaches to a running host for ten seconds — Chapter 36's listing is in its shape, the `(in meter.plugin)` module column included (`[self]` is that chapter's own marker). Instruments' *Time Profiler* template is the same sampler with a UI: pick the running host in its target chooser, or from the command line `xctrace record --template 'Time Profiler' --attach <pid>` (`--launch -- ./app` to start one), which needs the full Xcode.
...
1. **Build the thing you ship, with the names kept.** Profile at `-O2` (`/O2` on MSVC) — an unoptimized profile is a profile of a different program, with copies and calls the optimizer would have removed — but keep `-g` (`/Zi`, and `/DEBUG` to the linker, which the IDE's Release configuration already passes) so frames have names; `-g` changes no instruction at `-O2`. The symbol file is the same artifact [Chapter 37](37-no-repro-dump-attached.md#chapter-37--no-repro-dump-attached) archives for crash reports; a profile of a stripped binary is a column of addresses for the same reason a stripped crash report is — and for [Chapter 40](40-cmake-for-the-plug-in.md#chapter-40--cmake-for-the-plug-in)'s plug-in, hidden by preset with one export, it is one name and offsets from it.
2. **Keep the frame pointers.** Add `-fno-omit-frame-pointer` on clang and GCC for x86-64 — `-O2` drops the register there. Without it a sampler walking the stack by frame pointer truncates or garbles the callers, and a profile in which every function appears to be called from nowhere, or from somewhere absurd, is the tell. macOS on arm64 keeps the frame pointer by ABI, and MSVC's x64 code keeps unwind tables the profiler walks regardless, so the flag is a Linux and Windows-on-clang concern.
