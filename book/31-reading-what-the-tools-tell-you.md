...
| The request returned OK and the body is an HTML error page — the JSON parse fails and "the server is flaky" | Recipe 41 in [Appendix F](F-rosetta-cookbook.md#appendix-f--the-rosetta-cookbook): two verdicts, the transport's and the server's, and only one was checked |
