...
1. **A runtime flag.** A `bool` read from configuration *once, at startup*, into a member, and tested with `if` — a branch, which is free even on [Chapter 36](36-the-host-stutters.md#chapter-36--dropouts-with-the-plug-in-loaded)'s deadline thread, where a lookup per call is not a flag but a crime. For anything that changes behavior and not types it is the best tool, because no build is involved at all; reaching for the preprocessor first is a habit from before configuration files. Recipe 31 in [Appendix F](F-rosetta-cookbook.md#appendix-f--the-rosetta-cookbook) is the shape — a struct of defaults filled once at startup.
...
Two rules ride on the tree that no file states, and both are habits you already have — one type per file is C#'s, and a namespace that follows the folder is what Visual Studio writes into every new file — with one difference that makes them rules here rather than defaults: the C# compiler finds a type wherever its file sits, and a C++ `#include` path is spelled by hand, so a name that means three things in the path, the namespace and the file costs a search every time. **The namespace mirrors the directory** — Boost's and Google's habit, not a standard's: when the tree grows a `wire/` for the parser of [Chapter 34](34-parse-this-capture.md#chapter-34--every-capture-rejected-as-malformed), the code under `include/myplugin/wire/` lives in `namespace myplugin::wire`, so an identifier's home is readable off its qualified name and a `#include` path and a `using` never disagree about what a thing is called. And **one class, one header pair:** `Session` is declared in `session.h` and defined in `session.cpp`, named for the type — spelled however the codebase spells its files (Appendix A.8; this book's labs use `Greeter.h`, deplab uses `mathlib.h`) — so a reader who meets the type in a review can open its file without a search. Private headers — the ones nothing outside `src/` may include — stay in `src/` beside the `.cpp` that owns them, and never gain the project-name prefix: `<myplugin/...>` is the ABI surface of the tree above, and everything [Chapter 30](30-authoring-an-abi-boundary.md#chapter-30--authoring-an-abi-boundary) says about a boundary being a promise applies to it and to nothing in `src/`. Tests sit apart in `tests/`, named for what they test (`session_test.cpp`): a test binary is a second executable with its own `main`, so it can never be a source of the first one ([Chapter 28](28-testing.md#chapter-28--testing)), and a directory of its own keeps the two source lists from being confused by the person, since CMake will not confuse them. Both rules cost nothing on day one and a rename on day ninety; the tree, the root files and the two rules together are what to copy — and `exercises/skeleton/` is that copy, the tree above as a directory with the root files in place bar `LICENSE`, which the repository's own covers, and which `build_all.sh` configures through its preset, builds and tests, and CI holds to its own `.clang-format` and `.clang-tidy`. The labs beside it are not formatted by machine, because their layout — one-line structs, aligned comments, a short body on the line that declares it — is part of what the chapters quote.

### Do dev/uat/prod differences belong in the CMake build configuration, or at runtime?

They look like the same kind of choice — "which variant am I building" — and CMake gives you a mechanism, `CMAKE_BUILD_TYPE` and a preset, that is tempting to reuse for it. They are not the same axis. A **build configuration** (Debug, Release, `GREETER_SANITIZE=ON`) changes what the compiler emits, so it is fixed the moment `cmake --build` finishes and a different one means a different binary. An **environment** (dev, uat, prod: which endpoint to call, which public key to trust) changes nothing about the code — the same binary is correct in all three — so it belongs where Recipe 31 in [Appendix F](F-rosetta-cookbook.md#appendix-f--the-rosetta-cookbook) already put a feature flag: read once at startup, into a struct, from whatever channel the deployment already has (an environment variable here; a JSON file, the host's own preferences API, or a command-line argument are the same shape):

```cpp
--8<-- "exercises/questions/q2.cpp:answer"
```

Building a *second* binary per environment — a preset per environment, `-DAPP_ENDPOINT=...` baked in with `target_compile_definitions` — is what produces the stuck symptom: a preset's cache variables are written into `CMakeCache.txt` inside that preset's `binaryDir` at *configure* time (the next section's "stale cache" pitfall), so switching a preset and rebuilding without a fresh configure of that directory changes nothing, because nothing asked CMake to write the new value down. The fix is not a cache workaround; it is not encoding an environment as a build configuration at all. The one CMake-level choice that *is* a build configuration in this story is whether debug/sanitizer/release differ between environments, and that is what `CMakePresets.json` is for — and, since CMake 3.19, Visual Studio and Visual Studio Code's CMake Tools extension both read `configurePresets`/`buildPresets` from that one file natively, so picking `dev` in either IDE's preset dropdown is exactly `cmake --preset dev` on the command line. Commit `CMakePresets.json`; anything local — a different generator, a path only one machine has — goes in `CMakeUserPresets.json`, which both IDEs also read and which stays out of version control, the same split C# keeps between `appsettings.json` and `appsettings.Development.json`.

**In C#:** this is `ASPNETCORE_ENVIRONMENT` plus `appsettings.{Environment}.json` bound through `IConfiguration`/`IOptions` — one build, the environment selected by what is loaded at startup, never by what `dotnet build` was asked for; a secret follows the same rule via `dotnet user-secrets` or a vault, never committed, which is what `CMakeUserPresets.json` (or a channel that never touches disk at all) stands in for here.

> [!TIP]
> **Habit:** I keep "which build" (compiler flags, one `CMakePresets.json` read by every IDE) and "which environment" (endpoints, keys — a value read once at startup) on two separate axes, and I never reach for a `#define` to cross an environment boundary a `getenv` already crosses for free.

### Why doesn't switching a CMake preset always change what gets built?

The previous section's two IDEs both read the same `CMakePresets.json` — but "reads the file" and "the switch takes effect" are two different claims, and when the presets are legitimately different builds (Debug, Release, a sanitized build — not a disguised environment) the gap between them has three distinct causes. Cheapest check first:

**Rule out the file being read at all.** `CMakePresets.json` integration is off by default in Visual Studio (Tools > Options > CMake > General); Visual Studio Code's CMake Tools extension turns it on the moment the file exists, but Visual Studio does not. With the option off, Visual Studio drives configure and build entirely from any `CMakeSettings.json` still sitting in the folder — present or not, that older file wins over `CMakePresets.json` until the option is flipped on — so every edit to the presets file, and the picker built from it, is inert. Turn the option on and close and reopen the folder to make it take; then delete the leftover `CMakeSettings.json`, or a teammate who hasn't flipped the same switch silently falls back to it again.

**Rule out the generator next.** A preset's `generator` field decides whether `CMAKE_BUILD_TYPE` means anything at all. Visual Studio's own generator (and Xcode) is *multi-config*: every configuration is generated side by side and the choice is made at *build* time — the toolbar's Debug/Release dropdown, `--config` on the command line — so `CMAKE_BUILD_TYPE` is read by nothing (the Pitfalls entry below is the same fact met from a raw command line rather than a preset). Ninja and Makefiles are *single-config*: the whole `binaryDir` commits to one configuration at *configure* time, which is why Visual Studio Code's CMake Tools extension — it prefers Ninja when one is on PATH — reacts to a preset the way the maintainer expects while Visual Studio's own default generator does not. A `CMakeLists.txt` line that tests `CMAKE_BUILD_TYPE` directly (`if(CMAKE_BUILD_TYPE STREQUAL "Release") target_compile_definitions(...)`) inherits this split unchanged: the branch never taken under the multi-config generator, taken correctly under the single-config one. A generator expression — `$<CONFIG:Release>` and its kin, evaluated at *build* time — reads the same under both kinds of generator, and is the fix wherever a preset needs to reach a define.

**Rule out the cache last**, because it needs the other two ruled out to mean anything. Every preset's `cacheVariables` entry is written into `CMakeCache.txt` inside that preset's `binaryDir` with `-D` semantics — it overwrites unconditionally, every configure — but only for the variables that preset actually lists. Two presets sharing one `binaryDir`, where only one of them mentions the variable that tells them apart, leave that variable at whatever the *other* preset last wrote: switching back looks like nothing happened, because for that one variable nothing did — the "VS Code keeps building the first preset" shape of the symptom. Two fixes, and take both: give every preset its own `binaryDir` (`${sourceDir}/build/${presetName}` is CMake's own default template's shape, and it turns "switch preset" into "configure a clean directory" instead of "reuse a cache"), and have every preset that participates in the switch set the variable explicitly — never let one preset's value be "whatever was last there."

That last habit generalizes past CMake: a build-variant switch with an implicit default is one state a stale cache can reuse without anyone asking for it. Make the set exhaustive and let the compiler catch a variant nobody picked, instead of silently keeping whichever one configured first:

```cpp
--8<-- "exercises/questions/q3.cpp:answer"
```

**In C#:** MSBuild sidesteps the shared-cache failure mode entirely — Debug and Release each get their own `obj/Debug` and `obj/Release`, so switching the toolbar's configuration can never leave a stale intermediate from the other one lying around to be picked up by accident. Giving every CMake preset its own `binaryDir` is the same separation, done by hand, because CMake does not impose it.

> [!TIP]
> **Habit:** when a preset switch "does nothing," I check in this order — is the IDE actually reading `CMakePresets.json`, does the thing that differs test `CMAKE_BUILD_TYPE` under a multi-config generator, and only then do I suspect the cache — and every preset that shares a `binaryDir` with another sets every variable that tells them apart, never just the non-default one.
