...
**Try it (30 seconds).** Predict where each of the five lives, then print `&x`, `&w`, `p`, `u.get()` and `v.data()`: the addresses sort themselves into two distant neighbourhoods, and seeing your own stack and heap once beats any diagram.
...
One answer C# never had to give: *because it does not fit*. A class instance in C# is on the heap whether it holds forty bytes or forty megabytes; here a `std::array` of four megabytes is four megabytes of the object that holds it, and of the frame of the function that holds *that*, and the frame has to fit inside the stack sized above — on a Windows or macOS worker thread, a single such local is a crash on entry to the function, before its first line runs. The spelling that puts the bytes on the heap and leaves a pointer-sized owner behind is `std::make_unique<Big>()` — Chapter 1's `unique_ptr`, chosen for size rather than lifetime — and Recipe 34 in [Appendix F](F-rosetta-cookbook.md#appendix-f--the-rosetta-cookbook) is the shape, with the `static_assert` that keeps the reason for the heap written down.
...
- use-after-move where the operation has a *precondition* — dereferencing a moved-from `unique_ptr` (guaranteed empty by the standard, so that is exactly a null dereference), or `front()` on a moved-from vector (a husk that is valid but *unspecified*: `size()` or reassigning it is fine, `front()` is not); the discipline either way is assign-or-destroy (Chapter 6)
...
`int x;` *default-initializes*, and for built-in types default-initialization does nothing at all. The value is **indeterminate**; reading it is undefined behavior of the quiet kind. Debug builds often show 0 on Linux and macOS, because pages fresh from the OS are zeroed; MSVC's Debug runtime checks (`/RTC1`, on by default in the Visual Studio Debug configuration) fill the slot with a pattern instead — `0xcccccccc`, which reads back as `-858993460`, the most recognizable Windows tell that nobody wrote to this variable. For a plain local like `x` you never get that far, because `/RTC1` is `/RTCs` (the fill) plus `/RTCu` (the report), and `/RTCu` stops the program outright with *Run-Time Check Failure #3 — The variable 'x' is being used without being initialized*. Take that as a gift rather than the rule, because `/RTCu` gives up on a variable the moment its address is taken — so aliased locals, members and array elements hand you the pattern in silence. All of it is an accident of the build rather than a guarantee, and Release shows whatever that stack slot last held. That is exactly the works-on-my-machine signature this chapter opened with, and the reason it is worth its own section: the bug does not announce itself, and you have no instinct that says to look.
