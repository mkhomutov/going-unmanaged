...
- **And the rule does not stop at a function call.** Bytes another process reads — a wire ([Chapter 34](34-parse-this-capture.md#chapter-34--every-capture-rejected-as-malformed)), a shared-memory region (Recipe 43 in [Appendix F](F-rosetta-cookbook.md#appendix-f--the-rosetta-cookbook)) — cross a boundary too, and the same sentence governs them: fixed-width fields, a version first, no pointers, nothing the compiler laid out.
