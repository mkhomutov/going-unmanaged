...
Split a trivial `Greeter` class across `Greeter.h` / `Greeter.cpp` with a `main.cpp` consumer. Get it building. Then break it eight ways, one at a time, and for each: predict the error *stage* (preprocessor / compile / link), provoke it, and paste the first error line into your notes with a one-line translation.
...
8. **Make the header borrow an include from its own .cpp.** Delete `#include <string>` from Greeter.h, and in Greeter.cpp put `#include <string>` *above* `#include "Greeter.h"`. Greeter.cpp still compiles — the header's `std::string` is declared by the time the paste reaches it — and main.cpp, which includes Greeter.h first, fails inside the header with `use of undeclared identifier 'std'`. The header was never self-sufficient; its own .cpp hid that by feeding it the include. The rule from Chapter 12 exists for this: a .cpp includes its own header **first**, so the failure lands in the file whose author can fix it. Put the include back in the header and swap the order in Greeter.cpp, and note that the error now appears exactly once, in the right place.
