...
**Named — and the one thing it does that C#'s does not.** `namespace acme { ... }`, `acme::Widget`, `using namespace acme;`, and `namespace fs = std::filesystem;` for an alias. Two rules that are not C#'s. `using namespace` at file scope in a **header** drags every name it holds into every translation unit that includes it, forever, and is the one genuinely reviled line in this area — inside a `.cpp`, or inside a function, it is fine. And a namespace does more here than shorten typing: **argument-dependent lookup** means an unqualified call also searches the namespaces of its *arguments'* types, which is why `swap(a, b)` finds the right `swap` and why Recipe 25's JSON conversions are two free functions in your type's own namespace rather than anything registered anywhere. ADL is the mechanism behind half the standard library's customization, and the reason a helper written beside a type is found by code that never heard of it.

**Unnamed — the file-private one.** A `namespace { ... }` with no name gives everything inside it **internal linkage**: the name exists in this translation unit and in no other, so two `.cpp` files may each define `clamp_to_range` with different bodies and the linker is never asked to choose. That is where a helper nobody else calls belongs — the row the table above gave it — and it is the closest thing C++ has to `internal`, except that the unit is the file rather than the assembly. `static` at namespace scope says the same thing and is the older spelling you will read in C code; the unnamed namespace is preferred because it works for types too, which `static` cannot do. Recipe 50 in [Appendix F](F-rosetta-cookbook.md#appendix-f--the-rosetta-cookbook) is the shape, demonstrated across the two files it takes to demonstrate. The one thing that breaks it is a **unity build** ([Appendix J](J-cmake-catalogue.md#appendix-j--the-cmake-catalogue)), which concatenates translation units before compiling them: the two files become one, and two file-private names that never met now collide.
