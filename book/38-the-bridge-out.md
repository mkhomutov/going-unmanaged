...
- **The transport menu, priced.** WebSocket plus JSON-RPC is the desktop-bridge sweet spot — one small dependency, bidirectional, browsers connect natively. gRPC buys generated clients and streaming for several languages you own, and costs a heavy dependency tree that must be static-linked and symbol-hidden or it will collide with the host's own copies ([Chapter 27](27-dependency-management.md#chapter-27--dependency-management)'s diamond, in the wild). Named pipes or Unix sockets when the dependency budget is zero. Plain HTTP when request/response is genuinely all there is. The catalogue — every option, priced, with the decision table — is [Appendix G](G-the-bridge-catalogue.md#appendix-g--the-bridge-catalogue).
