...
The const overload returns `const int&` for the reason the whole page rests on: handing back a writable reference from a const member function would give the caller a non-const path to an object they reached through a const one. The promise would be intact in the signature and broken in fact. [Finding 8 of Chapter 25](25-findings-from-practice.md#chapter-25--gotchas-the-findings-log) is this pair found the hard way, on the Chapter 15 Buffer — including the half the compiler will not enforce for you.
