...
- **Part VI (Chapter 26 onward):** the real codebase — what a project has that an exercise does not, starting with build systems and dependency management. This is where the roadmap's appended chapters land.
...
4. **Missing chapters** — subjects the book does not cover yet. [ROADMAP.md](ROADMAP.md) is the standing list, ranked by what they cost a reader who hits them unprepared: testing and concurrency are the biggest still open (build systems landed as Chapter 26, dependency management as Chapter 27). Open an issue before starting a large one.
