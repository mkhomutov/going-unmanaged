...
**Do not take that on trust — it is the part you can check.** [`scripts/build_all.sh`](scripts/build_all.sh) is that sanitizer run, one command on your machine and the same one in CI; [`scripts/check_verbatim.sh`](scripts/check_verbatim.sh) diffs the chapters against the lab, solution and vendor files they quote, byte for byte, so a chapter cannot drift from the code it prints; [`scripts/check_platform_claims.sh`](scripts/check_platform_claims.sh) runs the sanitizer demonstrations and asserts what the chapters promise *per platform*, on Linux and macOS both, because a claim true on one is not a rule; and [`.github/workflows/ci.yml`](.github/workflows/ci.yml) is where all of it runs on every push. The honest limit of that: it checks the code, and the claims a program can make. The prose is checked by re-deriving it against a compiler and the vendor docs — the same way you would fact-check a chat window — and [CHANGELOG.md](CHANGELOG.md) records the rounds where that found something, including the round where the corrections themselves needed correcting. [CONTRIBUTING.md](CONTRIBUTING.md#assistants-and-the-exercises) carries that method as a rule, for your assistant as much as for this one.
...
- **Appendices:** a fundamentals refresher, a one-page cheat sheet, curated resources, a glossary, the Rosetta Cookbook indexed by the C# API you are reaching for — each recipe in C++ and, increasingly, in Rust, both included from tested code — the bridge catalogue for the day the plug-in must speak to everything else, and the choosing procedures for the four decisions every signature makes.
...
**What to assign.** The [exercise index](exercises/README.md) is the order: the Tracer, the Buffer and the Word Counter, then the two SDK labs — against *your* SDK if they have access to it, the fake SDKs standing in if not — each done cold with `scripts/check.sh` as the judge. It is about a week of work, and it front-loads the mistakes you would otherwise meet in code review. After that, the ticket chapters ([32](book/32-it-crashes-on-exit.md) onward) are shaped like the tickets you will actually assign: symptom first, diagnosis before the fold, an acceptance test that proves the fix's actual claim.
...
3. **New exercises** — especially new SDK shapes. The two longest-standing gaps are now closed (COM-style refcounting is Chapter 35 and `exercises/comlab/`; the threaded callback is `exercises/threadlab/`), and the frameworks that bring their own object model — [ROADMAP item 18](ROADMAP.md#18-the-framework-shape--bestiary-shape-5-taught--done-chapter-44) — are now Chapter 44 and `exercises/framelab/`, so no Bestiary shape is without a treatment.
4. **Missing chapters** — subjects the book does not cover yet. [ROADMAP.md](ROADMAP.md) is the standing list, ranked by what they cost a reader who hits them unprepared: Tier 1 is closed (Chapters 26-29), and the ticket arc has grown to six (Chapters 32-37, with the performance and crash-dump tickets the latest — the format stays open to new tickets by PR). P/Invoke landed as Chapter 39, const-correctness as Appendix I, the glossary as Appendix E, and the hand-off to a deadline thread as Chapter 43; the framework shape as Chapter 44 and the retrofit as Chapter 45; SOLID without the runtime (item 13) is the open chapter. Open an issue before starting a large one.
...
That list being one name long is the current state of this project, not a claim about it. Everything here has had exactly one human reviewer, working with an AI. It is why the mechanical checks above exist — they catch what one pair of eyes will not — and why a [Finding](book/25-findings-from-practice.md) from your own practice is the shortest path onto the list.
