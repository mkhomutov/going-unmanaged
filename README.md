...
3. **New exercises** — especially new SDK shapes. The two longest-standing gaps are now closed (COM-style refcounting is Chapter 35 and `exercises/comlab/`; the threaded callback is `exercises/threadlab/`), which leaves the frameworks that bring their own object model — [ROADMAP item 18](ROADMAP.md#18-the-framework-shape--bestiary-shape-5-taught) — as the one Bestiary shape with no treatment anywhere.
